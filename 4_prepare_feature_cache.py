...
def splatting_target_mask_kernel(
    ...
    max_x = min(
        grid_dims[0], int(((pos_x + max_cutoff_target) - grid_min[0]) / res) + 1
    )
    min_y = max(0, int(((pos_y - max_cutoff_target) - grid_min[1]) / res))
    max_y = min(
        grid_dims[1], int(((pos_y + max_cutoff_target) - grid_min[1]) / res) + 1
    )
    min_z = max(0, int(((pos_z - max_cutoff_target) - grid_min[2]) / res))
    max_z = min(
        grid_dims[2], int(((pos_z + max_cutoff_target) - grid_min[2]) / res) + 1
    )
    for z_idx in range(min_z, max_z):
        for y_idx in range(min_y, max_y):
            for x_idx in range(min_x, max_x):
...
class SplattingFieldCalculator:
            ...
            for line in f:
                if line.startswith("HETATM") and line[17:20].strip() in names:
                    try:
                        ...
                        elem = (
                            line[76:78].strip().upper() or line[12:14].strip().upper()
                        )
                        atoms.append(
                            Atom(coords, VDW_RADII.get(elem, VDW_RADII["DEFAULT"]))
                        )
                    except (ValueError, IndexError):
                        continue
        return atoms
    ...
    def calculate_all_fields(
        ...
        grid_dim_3d = tuple(
            (s + b - 1) // b for s, b in zip(dims_zyx, CUDA_BLOCK_SIZE_3D)
        )
        splatting_dilation_kernel[grid_dim_3d, CUDA_BLOCK_SIZE_3D](
        ...
        if ligand_atoms:
            ligand_coords_d = cp.asarray(
                [a.coords for a in ligand_atoms], dtype=cp.float32
            )
            ligand_grid_size_1d = (
                len(ligand_atoms) + CUDA_BLOCK_SIZE_1D - 1
            ) // CUDA_BLOCK_SIZE_1D
...
def process_and_visualize_task(
    ...
    if ligand_atoms:
        ...
        voxel_center = np.round(
            (center_coords - grid_params.origin) / grid_params.resolution
        ).astype(int)
    ...
    all_channels = CHANNELS + ["target_mask"]
    all_data = np.concatenate((features_f32, target_mask[np.newaxis, ...]), axis=0)

    cmaps = {
        "hydrophobicity": "seismic",
        "electrostatic": "bwr",
        "aromatic_field": "viridis",
        "shape_index": "PRGn",
        "hbond_donor": "Reds",
        "hbond_acceptor": "Blues",
        "occupancy": "plasma",
        "sasa": "magma",
        "target_mask": "hot",
    }
    ...
    for i, ax in enumerate(axes.flat):
        ...
        data_slice = all_data[i, :, :, slice_coord].T

        vmin, vmax = (
            (-1, 1)
            if channel_name in ["hydrophobicity", "electrostatic", "shape_index"]
            else (0, 1)
        )

        im = ax.imshow(
            data_slice,
            cmap=cmaps.get(channel_name, "gray"),
            origin="lower",
            vmin=vmin,
            vmax=vmax,
        )
        ax.set_title(channel_name.replace("_", " ").title(), fontsize=10)
        ...
        cbar = fig.colorbar(im, ax=ax, shrink=0.7, aspect=10)
        cbar.ax.tick_params(labelsize=8)

    plt.savefig(vis_dir / f"{pdb_id}_fields_slice.pdf", bbox_inches="tight")
    plt.close(fig)


def update_performance_plot(
    performance_data: List[Tuple[int, float]], output_path: Path
):
    ...
    ax.scatter(atom_counts, times_s, color="black", alpha=0.7, edgecolors="none", s=20)
...
def main() -> int:
    ...
    with ProcessPoolExecutor(max_workers=NUM_PROCESS_WORKERS) as executor:
        for name, config in DATASET_CONFIGS.items():
            ...
            try:
                ...
                console.log(
                    f"[yellow]Warning: Could not find input file {config['csv_file']} or directory {config['pqr_dir']}. Skipping dataset.[/yellow]"
                )
                continue
            ...
            with Progress(
                ...
                task = progress.add_task(
                    f"Caching [cyan]{name}[/cyan]", total=len(pqr_files)
                )
                ...
                for pqr_path in pqr_files:
                    ...
                    try:
                        ...
                        ligand_names = []
                        if (
                            "ligands" in df_pdb.columns
                            and pdb_id in df_pdb["pdb_id"].values
                        ):
                            ligand_str = df_pdb.loc[
                                df_pdb["pdb_id"] == pdb_id, "ligands"
                            ].iloc[0]
                            if pd.notna(ligand_str):
                                ligand_names = [
                                    name.strip() for name in ligand_str.split(";")
                                ]

                        ligand_atoms = calculator.parse_ligand_from_pdb(
                            pdb_path, ligand_names
                        )

                        start_calc_time = time.perf_counter()
                        results = calculator.calculate_all_fields(
                        ...
                        update_performance_plot(
                            performance_data, OUTPUT_ROOT / "performance_plot.pdf"
                        )

                    except Exception as e:
                        console.log(
                            f"[bold red]Error processing {pdb_id}: {e}[/bold red]"
                        )
    ...
    console.print(
        f"Total execution time: {total_time / 60:.2f} min ({total_time:.2f} sec)."
    )
    console.print(
        f"Cache and visualizations saved in: [cyan]{OUTPUT_ROOT.resolve()}[/cyan]"
    )
    return 0
