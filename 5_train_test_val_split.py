...
def create_final_split_report():
    ...
    REPORT_POSITIVE_CSV = Path(
        "protein_clustering_results/clustered_protein_report.csv"
    )
    ...
    for group_name, report_path in reports.items():
        ...
        try:
            ...
            console.log(
                f"[yellow]Warning: Report file not found, skipping: {report_path}[/yellow]"
            )
            continue
        ...
        console.print(
            f"  - Assigning to test: {n_total - len(train_clusters) - len(val_clusters)} clusters"
        )
        ...
        all_split_data.extend(
            df[["pdb_id", "cluster_id", "split", "group"]].to_dict("records")
        )

    if not all_split_data:
        console.print(
            "[bold red]Error: No data was processed. Check input file paths.[/bold red]"
        )
        return
    ...
    console.print(
        f"Final report with split assignments saved to: [bold cyan]{OUTPUT_CSV}[/bold cyan]"
    )

    console.print("\n[bold]Split Summary (by number of proteins):[/bold]")
    summary = (
        final_df.groupby(["split", "group"])["pdb_id"].count().unstack(fill_value=0)
    )
