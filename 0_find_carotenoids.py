...
PATTERNS = {
    "head_to_tail": Chem.MolFromSmarts(
        "[#6]-[#6](-[CH3])=[#6]-[#6]=[#6]-[#6](-[CH3])=[#6]"
    ),
...
def download_ccd_file():
    ...
    try:
        ...
        console.print(
            f"[bold red]Error during download or decompression: {e}[/bold red]"
        )
        if CCD_FILE.exists():
        ...
        if bond_atom1s:
            ...
            for i in range(len(bond_atom1s)):
                ...
                bond_type = BOND_TYPE_MAP.get(
                    bond_orders[i].upper(), Chem.BondType.UNSPECIFIED
                )
...
def main():
    ...
    if ignore_set:
        console.print(
            f"Ignoring [yellow]{len(ignore_set)}[/yellow] codes from the blacklist."
        )
    ...
    progress = Progress(
        TextColumn("[cyan]Analyzing components..."),
        BarColumn(),
            ...
            for block in doc:
                ...
                if mol:
                    ...
                    if match_types:
                        ...
                        try:
                            MolToImage(mol, size=(300, 300), kekulize=True).save(
                                img_path
                            )
                        except Exception:
                            pass
    ...
    if not found_ligands_data:
        console.print(
            "[yellow]No polyterpene ligands found (after applying filters).[/yellow]"
        )
        return
    ...
    console.print(
        f"Molecule images saved in: [bold cyan]{OUTPUT_IMAGE_DIR}[/bold cyan]"
    )
    ...
    if synonym_groups:
        ...
        for group in synonym_groups:
            ...
            console.print(
                f"  - Group [bold green]{codes[0]}[/bold green]: {', '.join(codes)}"
            )
