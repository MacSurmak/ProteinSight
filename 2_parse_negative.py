...
def process_dataset(title: str, records: list[dict], base_dir: Path):
    ...
    with Progress(console=console) as progress:
        ...
        meta_task = progress.add_task(
            "Fetching metadata...", total=len(pdb_ids_to_process)
        )
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = {
                executor.submit(fetch_and_parse_details, pid)
                for pid in pdb_ids_to_process
            }
            metadata = [
        ...
        dl_task = progress.add_task(
            "Downloading CIF files...", total=len(pdb_ids_to_process)
        )
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = {
                executor.submit(download_structure_file, pid, cif_dir)
                for pid in pdb_ids_to_process
            }
        ...
        parse_task = progress.add_task(
            "Extracting sequences...", total=len(structure_files)
        )
        all_sequences = {}
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = {
    ...
    if cluster_df is not None:
        ...
        pdb_to_cluster = cluster_df.drop_duplicates(subset="pdb_id").set_index(
            "pdb_id"
        )["cluster_id"]
        report_df["cluster_id"] = (
            report_df["pdb_id"].map(pdb_to_cluster).fillna("No_Cluster")
        )
    else:
    ...
    console.print(
        f"[bold green]Report for '{title}' saved to: {report_file}[/bold green]"
    )
    console.print(
        f"Total structures: {len(report_df)}, Unique clusters: {report_df['cluster_id'].nunique()}"
    )
...
def main():
    ...
    for group_name, data in NEGATIVE_CONTROL_TARGETS.items():
        ...
        console.print(
            f"Randomly selected {len(selected_ids)} structures for this group."
        )
    ...
    console.print(
        f"Found {len(albumin_pdb_ids)} structures for '{ALBUMIN_TARGET['search_term']}'."
    )
    ...
    albumin_records = [
        {"pdb_id": pid, "group": "albumin"} for pid in selected_albumin_ids
    ]
